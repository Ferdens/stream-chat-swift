...
private enum TraitCollectionWatcher {
    ...
    private static var activeTap: Wiretap!

    static func activate() {
        guard activeTap == nil else { return }

        activeTap = Wiretap(frame: CGRect(x: -10, y: -10, width: 1, height: 1))
        activeTap.backgroundColor = .clear
        activeTap.onTraitChange = {
            DispatchQueue.main.async {
                TraitCollectionWatcher.dfsAppearanceReset()
            }
        }

        UIApplication.shared.keyWindow?.addSubview(activeTap)
    }
